PanNETsubtypesmRNAVal <-function(Indata,IndSAMdata)
{
  ...
  dir.create("PanNETassignerResults/mRNAVal", showWarnings = FALSE)
  dir<-getwd()   
  setwd(paste0(dir,"/PanNETassignerResults/mRNAVal"))
  ...
  nmfconsensus(input.ds=InputFileName_sam_med,k.init=2,k.final=6,num.clusterings=20,maxniter=500, error.function="euclidean",data=dataUnique)
  ...
  InputFileName_sam_med_NMF<-paste((strsplit(InputFileName_sam_med,".txt")[1]),"_NMF_ordered_5.txt",sep="")
  data_2<-cbind(dataUnique_1[,1],dataUnique_1[,m])
  write.table(data_2,InputFileName_sam_med_NMF, row.names=FALSE,sep="\t",quote=FALSE)
  sessionInfo()
  save.image("PanNETsubtypesmRNAVal.RData")
  setwd(dir)
}
